...
```
* **`df.isin()`** : Sirve para filtrar por varios atributos o valores de una columna, Retorna un booleano por lo que es recomendable para una mejor visualización aplicar una función de agregación al final o encerrarlo en un df.
```python
...
```
* **`df.sample()`** : Coje una parte de la data del DataFrame que nosotros indiquemos.
```python
...
* **`cufflinks.to_df(Figure)`** : Extraerá de un gráfico ``plotly`` guardado en una variable, el dataframe que contiene el gráfico.
...
* **`,subplots=True`** : Crea una gráfica por cada Categoria del DataFrame. En el caso de tener varios gráficos de un tipo alojadas en una variable podemos hacer muchas minigráficas.
  ```python
...
* **`,shared_xaxes=True, shared_yaxes=True`** : Fuerza el mostrar etiquetas en eje X & Y.
