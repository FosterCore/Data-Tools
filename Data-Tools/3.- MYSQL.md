...
*  **` TRUNCATE `** : Elimina todos los datos de la tabla (``TABLE``) excepto los nombres de las columnas.
    ```sql
        ...
        SELECT * FROM ventas
        WHERE venta > 1000;
        -- rellenamos datos a la tabla desde filtro de otra tabla (copia)
...
* **`WITH ROLLUP`** : Añade una suma total en la parte final inferior de la tabla de solo datos numéricos excluyendo los datos texto automáticamente. Según las columnas con las que agrupemos con `GROUP BY` en relación con las columnas del ``SELECT`` afectara solo para la visualización mas no la suma del total. Traera el total dependiendo las operaciones del que hay en el `SELECT` (si hay un MAX() no sumara y traerá valor MAX)<br><br>
 ...
 GROUP BY (columna) WITH ROLLUP
    ...
    * Por tipo de total:
        ...
        ```
* **`ALL`** : Itera con cada valor de la columna indicada (como un ciclo *for i in column*). Se usa para itera sub queries cuando estas no son filtradas por un dato sino por una lista de valores.
    ```sql
...
* **`DELIMITER`** : Configura el símbolo con el que sql entenderá como fin de bloque de código. Puede ser **" ; "** (como esta por defecto) ó **" $$ "** para procedimientos `CREATE PROCEDURE`. Una vez cambiado es recomendable regresarlo como estaba por defecto **" ; "**. **Para VS Code no es necesario cambiar el `DELIMITER`**
    ...
    ```
* **`SET`** : Se usa dentro del contexto de un `CREATE PROCEDURE` para manipular las columnas y hacer operaciones con ellas. también se usa para crear variables
    ```sql
        SET factor_mejora = (ventas_total/ventas_conteo) * 1.10;
        ...
        --SELECT @p_suma_venta;
    ```
* **`NEW.`** : Toma el ultimo valor ingresado en una tabla a la que indicaremos. Ejm: contexto de un TRIGGER
    ```sql
            UPDATE orden 
...
Estos Joins se rigen por las perspectiva de que index es el que vamos a tomar para nuestra union. Sea index izquierda, derecha, intersección o Cruzado (rellenar).

* **`INNER JOIN`** : Es el ``JOIN`` por defecto. Esta union mostrara los datos de solo la intersección de Tabla A y B. Solo arrojara en la union las filas que tengan datos tanto en las columnas de la  tabla_A y tabla_B. Por ello nuestras filas estarán salteadas solo en donde se cumpla esto.
  
* **`LEFT JOIN`** : Esta union mostrara todas las filas (index) de la tabla izquierda (Tabla A) y la tabla_B se acomoda a este orden impuesto del index Tabla_A. La tabla A es la que indicamos en el `FROM` y la tabla B es la que va después del `LEFT JOIN`. Rige el index (filas) por la Tabla_A. Las filas donde Tabla_A no tenga valores y si haya en Tabla_B dejara valores nulos.
  
* **`RIGHT JOIN`** : Esta union se regirá por el index de la tabla derecha (Tabla B) con la intersección. Rige el index (filas) por la Tabla_B.
  
* **`FULL JOIN`** : Esta union combinara ambos index y mostrara todos los datos de las tablas A y B incluyendo la intersección.

* **`CROSS JOIN`** : Combina todos los registros de una tabla con todos los registros de otra. Si los tamaños de las filas de las tablas difieren estas se completaran repitiéndose. En otras palabras, las filas donde Tabla_A no tenga valores y si haya en Tabla_B se rellenara con valores de Tabla_B repitiéndose.
  
Concatenar tabla debajo de otra:

* **`UNION`** : Concatena datos debajo de otros. Para ello se debe trabajar con la misma cantidad de columnas sino no se unirán, No es necesario que tengan el mismo nombre las columnas a adicionar (es recomendable que lleven el mismo nombre).<br>
El nombre de la nueva columna con los datos concatenados sera del primer `SELECT` por ello usamos `AS` para cambiarle el nombre.
    ```sql
...
``INSERT INTO`` Tabla (Columna) ``VALUES`` ("Dato1"),("Dato2")
    ...
    ```sql
        ...
        VALUES ("salchicha"), ("jamon"), ("salami")  #tablas y valores siempre entre ( ).
    ```
...
* **`DAYNAME(); MONTHNAME()`** : Arroja el nombre del día o mes.<br>
    ...
    ```
* **`EXTRACT()`** : Permite Arrojar (en formato numérico) el día o mes o año que le indiquemos de un campo de formato de fecha.<br>

    ```sql
        SELECT EXTRACT(MONTH FROM NOW());
        SELECT EXTRACT(MONTH FROM '2003-12-31 01:02:03');

        --Creamos una Query con una columna que nos muestre solo el año del campo fecha y la agrupamos por año.
        select nombre, compra, EXTRACT(YEAR from fecha)
        FROM tabla_a
...
CASE : si, WHEN : sentencia, THEN : operacion, ELSE : sino , END : Podemos asignarle nombre a toda la operación en columna

```sql
--La columna "Bono" tendrá 3 condiciones para el contenido de sus filas.
SELECT v.venta_empleado, v.venta,
CASE --Esta columna tendrá 3 condiciones
